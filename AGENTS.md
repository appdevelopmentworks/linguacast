...
Operating guide for Codex working on **linguacast**. Read this fully before writing code. Also read `docs/requirements.md`, `docs/architecture.md`, and `docs/session-plan.md`.
...
## Model policy (which Codex model to use)
...
3. **OpenRouter** (`https://openrouter.ai/api/v1`) — OpenAI-compatible cloud gateway. One key, model-agnostic (user picks a model slug like `anthropic/Codex-...`, `openai/gpt-...`, `google/gemini-...`). Do NOT build per-provider adapters in v0.1 — OpenRouter covers OpenAI/Anthropic/Gemini/etc. Used when both local options are unavailable or the user opts in for quality.
